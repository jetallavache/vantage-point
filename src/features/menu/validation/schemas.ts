...
export const menuTypeSchema = z.object({
  ...
  name: z
    .string()
    .min(1, errorMsg("Укажите название типа меню", ValidationErrorCodes.REQUIRED))
    .refine((val) => !hasSuspiciousContent(val), errorMsg("Обнаружено недопустимое содержимое", ValidationErrorCodes.SUSPICIOUS_CONTENT))
    .transform(sanitizeText),
});
...
export const menuItemSchema = z.object({
  ...
  name: z
    .string()
    .min(1, errorMsg("Укажите название пункта меню", ValidationErrorCodes.REQUIRED))
    .refine((val) => !hasSuspiciousContent(val), errorMsg("Обнаружено недопустимое содержимое", ValidationErrorCodes.SUSPICIOUS_CONTENT))
    .transform(sanitizeText),
  url: z
    .string()
    ...
    .optional()
    .or(z.literal("")),
  sort: z.number().min(0, errorMsg("Порядок сортировки не может быть отрицательным", ValidationErrorCodes.OUT_OF_RANGE)),
});
